...
// ValidateMessage performs pre-send validation of an ISO8583 message.
// Moved from the legacy REPL send path so App.Send and the REPL shim share
// one implementation.
...
func validateFinancialMessage(msg *iso8583.Message) error {
	...
	if err := validatePAN(msg); err != nil {
		return err
	}
...
// validatePAN checks the PAN (field 2) is numeric and 13-19 digits when present.
func validatePAN(msg *iso8583.Message) error {
