...
func VerifyTarget() error {
	...
	if strings.TrimSpace(host) == "" || strings.TrimSpace(port) == "" {
		return errors.New("target host and port are not configured. Please set target using 'target <host:port>' first")
	}
...
func VerifySpec(svc *service.Service) error {
	...
	if strings.TrimSpace(specFile) == "" {
		return errors.New("specification is not selected. Please select a specification using 'spec <path>' first")
	}
...
func VerifyTx(tx transactions.Repository) error {
	...
	if strings.TrimSpace(txFile) == "" {
		return errors.New("transaction file is not selected. Please select a transaction file using 'tx <path>' first")
	}
	if tx != nil && len(tx.ListNames()) == 0 {
		return fmt.Errorf(
			"transaction file '%s' contains no transactions. Please select a valid transaction file using 'tx <path>'",
			txFile,
		)
	}
