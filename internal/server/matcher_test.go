// matcher_test.go is the route-matching question: given a request, which route
// answers it, in what precedence, and with what composed response. The server's
// own lifecycle (start, stop, mTLS handshake) is in server_test.go.
package server
	...
	routes := []config.MockRouteConfig{
		{
			...
			ResponseMTI:    "0210",
			ResponseFields: map[string]any{"38": "123456", "39": "00"},
		},
	}
	...
	routes := []config.MockRouteConfig{
		{
			...
			MatchFields: map[string]any{
				"0": "0100",
				"2": []any{"4000111122223333", "4000222233334444"},
		...
		},
		{
			...
			MatchFields: map[string]any{
				"0": "0100",
				"2": map[string]any{
					"in": []any{"4000333344445555", "4000444455556666"},
				},
			},
			ResponseMTI:    "0110",
			ResponseFields: map[string]any{"39": "51"},
