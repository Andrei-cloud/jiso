...
// MTI+DE3 match, a catch-all, and an MTI-only match. Route 0's required
// list is deliberately unsorted — the detail's numeric sorter must put
// "3" before "11".
//
// MIRROR — keep in sync with serverRoutes() in pages/server_test.go: that
// literal is the pages-side copy of these rows (different packages, the
// import fence forbids a shared test source);
// TestRootServerDetailDerivation in root_server_detail_test.go pins the
// derived strings against it. Change one side, change the other.
func serveFixtureRoutes() []config.MockRouteConfig {
	return []config.MockRouteConfig{
		{
			Name: "0200/proc", Description: "purchase auth",
			MatchFields:    map[string]any{"0": "0200", "11": "000000", "3": "000000"},
			RequiredFields: []string{"11", "3"}, EchoFields: []int{11}, ResponseMTI: "0210",
			ResponseFields: map[string]any{"39": "00"}, DelayMs: 100, JitterMs: 25,
		},
		{Name: "0800/nmc", ResponseMTI: "0810", LatencyMs: 0},
		{
			Name: "0200/proc-mc", MatchFields: map[string]any{"0": "0200"},
...
func newServeTestRoot(t *testing.T) *serveTestRoot {
	...
	// The started server's own spec label, deliberately NOT the config's
	// spec ("../../specs/spec.json" above): a route detail that leaked the
	// global spec would fail the derivation pins.
	r.m.serveSpecFn = func() string { return "visa.json" }
