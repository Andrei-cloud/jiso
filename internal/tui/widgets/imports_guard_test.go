...
// TestWidgetsForbiddenImports enforces the leaf contract: widgets are
// composed BY internal/tui/frame and pages — they must never import the
// frame (composition goes one way), the CLI packages, or cobra. Imports
// are parsed, not grepped, so comments cannot mask a violation. os.Exit
// is scanned as a substring because the rule is about Update purity.
func TestWidgetsForbiddenImports(t *testing.T) {
	t.Parallel()
